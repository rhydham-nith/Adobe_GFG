...
def generate_triangle_points(num_points=100):
...
def generate_distorted_triangle_points(num_points=100, distortion_level=0.1):
...
distorted_triangle_points = generate_distorted_triangle_points(100, 0.1)

num_samples = 1000
triangle_samples = [generate_triangle_points() for _ in range(num_samples)]
non_triangle_samples = [generate_non_circle(100) for _ in range(num_samples)]

# Labels: 1 for circle, 0 for non-circle
labels = np.array([1] * num_samples + [0] * num_samples)

# Combine the data
data = np.array(triangle_samples + non_triangle_samples)
...
distorted_samples = [generate_distorted_triangle_points(100, 0.1) for _ in range(num_samples)]

# Combine all data
augmented_data = np.array(triangle_samples + non_triangle_samples + distorted_samples)
...
model_triangle.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

# Train the model with the augmented dataset
history = model_triangle.fit(X_train_aug, y_train_aug, epochs=4, batch_size=32, validation_data=(X_test_aug, y_test_aug))

import pickle

with open('model_triangle.pkl', 'wb') as file:
    pickle.dump(model_triangle, file)