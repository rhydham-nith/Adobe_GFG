...
def generate_star_points(num_points=100):
    # Calculate the coordinates of the pentagon vertices
    angle = 2 * np.pi / 5
    vertices = np.array([[np.cos(i * angle), np.sin(i * angle)] for i in range(5)])

    # Determine the number of points per diagonal
    points_per_diagonal = num_points // 5
    remaining_points = num_points % 5

    all_points = []
    for i in range(5):
        start = vertices[i]
        end = vertices[(i + 2) % 5]  # Connect every other vertex for diagonals
        diagonal_length = np.linalg.norm(end - start)
        step = diagonal_length / (points_per_diagonal + 1)

        for j in range(points_per_diagonal):
            point = start + (j + 1) * step * (end - start) / diagonal_length
            all_points.append(point)

    # Add remaining points to the first diagonal
    for i in range(remaining_points):
        start = vertices[0]
        end = vertices[2]
        diagonal_length = np.linalg.norm(end - start)
        step = diagonal_length / (num_points + 1)
        point = start + (points_per_diagonal + i + 1) * step * (end - start) / diagonal_length
        all_points.append(point)

    return np.array(all_points)


def generate_distorted_star_points(num_points=100, distortion_level=0.1):
    # Calculate the coordinates of the pentagon vertices
    angle = 2 * np.pi / 5
    vertices = np.array([[np.cos(i * angle), np.sin(i * angle)] for i in range(5)])
...
distorted_points = generate_distorted_star_points(100, 0.1)

num_samples = 1000
samples = [generate_distorted_star_points() for _ in range(num_samples)]
...
distorted_samples = [generate_distorted_star_points(100, 0.1) for _ in range(num_samples)]
...
model_star.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

# Train the model with the augmented dataset
history = model_star.fit(X_train_aug, y_train_aug, epochs=4, batch_size=32, validation_data=(X_test_aug, y_test_aug))

import pickle

with open('model_star.pkl', 'wb') as file:
    pickle.dump(model_star, file)