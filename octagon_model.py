...
def generate_octagon_points(num_points=100):
  # Calculate the coordinates of the octagon vertices
  angle = 2 * np.pi / 8
  vertices = np.array([[np.cos(i * angle), np.sin(i * angle)] for i in range(8)])
...
def generate_distorted_octagon_points(num_points=100, distortion_level=0.1):
  # Calculate the coordinates of the octagon vertices
  angle = 2 * np.pi / 8
  vertices = np.array([[np.cos(i * angle), np.sin(i * angle)] for i in range(8)])
...
distorted_points = generate_distorted_octagon_points(100, 0.1)

num_samples = 1000
samples = [generate_distorted_octagon_points() for _ in range(num_samples)]
...
distorted_samples = [generate_distorted_octagon_points(100, 0.1) for _ in range(num_samples)]
...
model_octagon.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

# Train the model with the augmented dataset
history = model_octagon.fit(X_train_aug, y_train_aug, epochs=4, batch_size=32, validation_data=(X_test_aug, y_test_aug))

import pickle

with open('model_octagon.pkl', 'wb') as file:
    pickle.dump(model_octagon, file)